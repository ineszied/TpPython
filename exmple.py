...
# --- Gestion des contacts avec la base de données MySQL ---
class Contact:
...
class GestionnaireContact:
    ...
    def connect_to_db(self):
        try:
            self.conn = mysql.connector.connect(
                ...
                database="contacts_db" # Nom de la base de données
            )
            if self.conn.is_connected():
                self.cursor = self.conn.cursor()
                print("Connexion réussie à la base de données.")
            else:
                print("Connexion échouée à la base de données.")
        except Error as e:
            print(f"Erreur de connexion à la base de données : {e}")

    def ajouter_contact(self, contact):
        query = "INSERT INTO contacts (nom, prenom, email, telephone) VALUES (%s, %s, %s, %s)"
        values = (contact.nom, contact.prenom, contact.email, contact.telephone)
    ...
    def afficher_contacts(self):
        ...
        return "\n".join([f"Nom: {row[0]}, Prénom: {row[1]}, Email: {row[2]}, Téléphone: {row[3]}" for row in rows])

    def rechercher_contact(self, nom):
        query = "SELECT nom, prenom, email, telephone FROM contacts WHERE nom = %s"
        self.cursor.execute(query, (nom,))
        rows = self.cursor.fetchall()
        return [f"Nom: {row[0]}, Prénom: {row[1]}, Email: {row[2]}, Téléphone: {row[3]}" for row in rows]

    def modifier_contact(self, nom, nouveau_contact):
        query = """UPDATE contacts
                   SET nom = %s, prenom = %s, email = %s, telephone = %s
                   WHERE nom = %s"""
        values = (nouveau_contact.nom, nouveau_contact.prenom, nouveau_contact.email, nouveau_contact.telephone, nom)
        self.cursor.execute(query, values)
        self.conn.commit()
        return self.cursor.rowcount > 0

    def supprimer_contact(self, nom):
        query = "DELETE FROM contacts WHERE nom = %s"
        self.cursor.execute(query, (nom,))
        self.conn.commit()
        return self.cursor.rowcount > 0
